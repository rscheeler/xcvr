...
from .conversions import temp2nf, nf2temp
from .symbols import Symbol
from .utils import xrnan2inf, net2da
from . import ureg
import base64
import markdown
...
class Device(object):
    ...
    def __init__(
        ...
        pn: str,
        network: Network,
        noise_network: Optional[Network] = None,
    ...
    def interpolate(self, frequency) -> None:
        ...
        for ia in interp_attrs:
            ...
            if isinstance(attr, Quantity):
                ...
                attr = xrnan2inf(attr.interp(frequency=frequency, kwargs={"fill_value": "extrapolate"}))
...
class System(object):
    ...
    def cascaded_p1db(self) -> xr.DataArray:
        ...
        for i in range(1, p1db.device.size):
            ...
            cp1dbn = 1 / (1 / (cp1db[-1].data * gain[dict(device=i)].data) + 1 / p1db[dict(device=i)].data)
            # Create DataArray
            cp1dbn = xr.DataArray(
                cp1dbn,
    ...
    def cascaded_oip3(self) -> xr.DataArray:
        ...
        for i in range(1, oip3.device.size):
            # Calculate for current stage
            coip3n = 1 / (1 / (coip3[-1].data * gain[dict(device=i)].data) + 1 / oip3[dict(device=i)].data)
            # Create DataArray
            coip3n = xr.DataArray(
                coip3n,
    ...
    def cascaded_iip3(self) -> xr.DataArray:
        ...
        for i in range(dsz)[-2::-1]:
            ciip3.append(ciip3[-1].drop_vars(["device", "manufacturer", "pn"]) / dgain.isel(device=i))
    ...
    def oip3(self) -> xr.DataArray:
        ...
        """
        oip3 = self._get_sys_prop("oip3", "OIP3", "dBm", "Output referred third order intercept point")
        return oip3
    ...
    def iip3(self) -> xr.DataArray:
        ...
        iip3 = self.oip3 / self.gain
        iip3.data = iip3.data.to("dBm")
        iip3 = self._get_sys_prop("iip3", "IIP3", "dBm", "Input referred third order intercept point")
        return iip3
    ...
    def block_diagram(self, input_dot: bool = True, output_dot: bool = True, **kwargs) -> schemdraw.Drawing:
        """
    ...
    def get_dataset(self, input_power: Optional[Quantity] = None, **kwargs) -> xr.Dataset:
        """
    ...
    def get_dataframe(self, input_power: Optional[Quantity] = None, **kwargs) -> DataFrame:
        """
    ...
    def get_signal_level(self, input_power: Union[Quantity, xr.DataArray]) -> xr.DataArray:
        """
